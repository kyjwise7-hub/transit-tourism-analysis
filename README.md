...
**학과 데이터분석 공모전 | 개인 프로젝트 | 2024.11 | 최우수상 수상**
...
<!-- 👉 outputs/figures/ 에 이미지 추가 후 아래 주석을 교체:
...
├── .gitignore
├── data/
│   └── data_description.md      # 변수 설명 (원본 CSV 미포함)
├── src/
│   └── 데이터분석_학과공모전.Rmd  # 전체 분석 코드
└── outputs/
    └── figures/                  # 주요 시각화 PNG
```
...
**실행:**
1. 이 레포를 클론합니다
2. `data/` 폴더에 `travel1.csv` 파일을 배치합니다 (변수 구성은 `data/data_description.md` 참고)
3. `src/데이터분석_학과공모전.Rmd`를 RStudio에서 열고 Knit 합니다
